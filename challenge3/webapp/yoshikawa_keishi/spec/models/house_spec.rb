...
describe House do
  describe '#create' do
    ...
    it "can't be saved without csv_id" do
      ...
      house.valid?
      expect(house.errors[:csv_id]).to contain_exactly("can't be blank")
    end

    it "can't be saved with a wrong type of csv_id" do
      house = build(:house, csv_id: "a")
      house.valid?
      expect(house.errors[:csv_id]).to contain_exactly("is not a number")
    end

    it "can't be saved without firstname" do
      house = build(:house, firstname: nil)
      house.valid?
      expect(house.errors[:firstname]).to contain_exactly("can't be blank")
    end

    it "can't be saved without lastname" do
      house = build(:house, lastname: nil)
      house.valid?
      expect(house.errors[:lastname]).to contain_exactly("can't be blank")
    end

    it "can't be saved without city" do
      house = build(:house, city: nil)
      house.valid?
      expect(house.errors[:city]).to contain_exactly("can't be blank")
    end

    it "can't be saved without num_of_people" do
      house = build(:house, num_of_people: nil)
      house.valid?
      expect(house.errors[:num_of_people]).to contain_exactly("can't be blank")
    end

    it "can't be saved without num_of_people" do
      house = build(:house, num_of_people: "a")
      house.valid?
      expect(house.errors[:num_of_people]).to contain_exactly("is not a number")
    end

    it "can't be saved without has_child" do
      house = build(:house, has_child: nil)
      house.valid?
      expect(house.errors[:has_child]).to contain_exactly("value should be only Yes(yes) or No(no)")
    end
